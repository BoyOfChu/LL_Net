...
def dice_val(y_pred, y_true, num_clus):
    with torch.no_grad():
        ...
        y_pred = y_pred.transpose((0, 4, 1, 2, 3))
        ...
        y_true = y_true.transpose((0, 4, 1, 2, 3))
        y_true = np.ascontiguousarray(y_true)
        # y_true = torch.squeeze(y_true, 1)
        # y_true = y_true.permute(0, 4, 1, 2, 3).contiguous()

        intersection = y_pred * y_true
        intersection = np.sum(intersection, axis=(2, 3, 4))
        union = y_pred.sum(axis=(2, 3, 4)) + y_true.sum(axis=(2, 3, 4))
        dsc = (2.*intersection) / (union + 1e-5)
    return np.mean(np.mean(dsc, axis=1))
